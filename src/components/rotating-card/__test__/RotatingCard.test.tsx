...
describe("RotatingCard", () => {
  test("frontComponent render on screen", () => {
    ...
    render(<RotatingCard frontComponent={<FrontComponent />} backComponent={<BackComponent />} />);
  ...
  test("backComponent not visible on screen", () => {
    ...
    const backElement = screen.getByText(/back\scomponent/i);
    expect(backElement).toBeInTheDocument();
    expect(backElement.parentElement).toHaveClass("wrapper__card__backComponent");
  });
});
